...
### hsv2rgb_scalar Overflow (>255)

When `v` (value) or `s` (saturation) parameters exceed 1.0, `hsv2rgb_scalar()` can produce R, G, B values > 255, causing `OverflowError: Python integer 265 out of bounds for uint8` when assigning to a uint8 array.
...
This commonly happens with particle systems where `brightness * density` can exceed 1.0, or when `v` in `hsv2rgb_scalar(h, 0.3, v)` is unclamped.

### Particle Grid Dimension Mismatch

When particles use module-level `COLS`/`ROWS` (from a default font size) but are rendered onto a grid with different dimensions, you get `IndexError` — the particle coordinates exceed the grid bounds.

```python
# WRONG — COLS/ROWS from one font size, grid from another
stars.update(t, density=0.7, drift=(0, -0.5))  # uses global COLS/ROWS
canvas_s = grid_sm.render(ch_s, co_s)           # grid_sm has different cols/rows

# RIGHT — pass grid dimensions explicitly
stars.update(t, cols=grid_sm.cols, rows=grid_sm.rows, density=0.7, drift=(0, -0.5))
canvas_s = grid_sm.render(ch_s, co_s)
```

Always pass `cols`/`rows` parameters to particle update functions matching the target grid, or parameterize the particle system to accept dynamic dimensions.

### Pickling Fails with Module-Level Mutable Globals

Even though all `fx_*` scene functions are defined at module level (picklable), `ProcessPoolExecutor` still fails if those functions reference module-level mutable objects (grid instances, particle systems, font objects) because the child process gets a different copy with uninitialized state:

```
_pickle.PicklingError: Can't pickle <function fx_void_emergence>: import of module 'sk' failed
```

**Root cause**: The child process re-imports the module, but module-level `GridLayer(...)` and similar initializers fail or produce different objects.

**Workarounds**:
1. **Sequential rendering** — drop `ProcessPoolExecutor` and render scenes in a loop. Acceptable for short videos (< 60s).
2. **Lazy initialization** — move all mutable object creation into functions, not module level. Each worker calls init functions on first use.
3. **Pass primitives, not objects** — serialize grid dimensions (int tuples) instead of GridLayer instances. Reconstruct in the worker.

For simple projects (< 5 scenes, < 500 frames), sequential rendering is the pragmatic choice.

### FeedbackBuffer Parameter Name Mismatch

When a scene config dict uses the key `blend` for feedback blend mode:

```python
"feedback": {"decay": 0.8, "blend": "screen", "opacity": 0.3, ...}
```

But `FeedbackBuffer.apply()` expects `blend_mode`, not `blend`. Passing `**fb_cfg` directly raises `TypeError: got unexpected keyword argument 'blend'`.

**Fix**: Map keys before calling:
```python
fb_kwargs = dict(fb_cfg)
if 'blend' in fb_kwargs:
    fb_kwargs['blend_mode'] = fb_kwargs.pop('blend')
canvas = fb.apply(canvas, **fb_kwargs)
```

### Container OOM During Render (exit 137 / exit -1)

When running in Docker with memory limits (e.g. 5GB), 1920×1080 rendering with multiple grids + FeedbackBuffer + ffmpeg encode can exceed the container's memory allocation. Symptoms: process killed by SIGKILL (exit 137), or background processes dying silently at exit code -1 with no stdout/stderr.

**Resolution strategy** (in priority order):
1. Drop resolution to 960×540 — still looks good for ASCII art, 4× less memory per frame
2. Reduce font sizes (smaller grids = fewer character cells = less memory for bitmap cache)
3. Reduce particle counts (stars, boids, etc.)
4. Skip GIF conversion (filter complex doubles memory); deliver MP4 only
5. Render scenes sequentially instead of in parallel
6. Add explicit `del` of intermediate canvases between scenes
