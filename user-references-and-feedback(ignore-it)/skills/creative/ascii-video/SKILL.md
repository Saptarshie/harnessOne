...
| Parallel | concurrent.futures | N workers for batch/clip rendering (⚠️ fall back to sequential if scene functions reference module globals — pickling fails) |
...
## Implementation Pitfalls (from field use)

These are the issues that burn real rendering sessions. Know them before you write the script.

### Resolution vs. Memory (OOM kills)

**1920x1080 with multiple grids + FeedbackBuffer will OOM in a 5GB Docker container.** The combination of per-frame canvases (~6MB), pre-rasterized character bitmaps (all chars × all grid sizes), feedback buffer (another frame), and ffmpeg's encode buffer routinely exceed 4–5GB. **Symptoms**: SIGKILL (exit 137) with no Python traceback, or background processes that die silently at exit code -1.

**Safe defaults per environment**:
| Environment | Max resolution | Grid sizes | Feedback? |
|------------|---------------|------------|-----------|
| 5GB Docker | 960×540 | 8/12/16px | Yes (safe) |
| 5GB Docker | 1280×720 | 10/16/20px | Avoid feedback |
| 8GB+ | 1920×1080 | 10/16/20px | Yes |
| Local (16GB+) | 1920×1080 | any | Yes |

If you hit OOM: reduce VW/VH, reduce font sizes, reduce star/particle counts, skip the GIF conversion step (GIF filter complex doubles memory), or render in sequential mode with explicit `del` of intermediate canvases.

### Parallel Rendering — Pickling Failures

`ProcessPoolExecutor` pickles scene functions and all their closure variables. **If scene functions reference module-level globals** (GridLayer instances, particle systems, star fields created at import time), pickling fails with `Can't pickle <function>: import of module failed`.

**Fix**: Render sequentially. For most projects (50s video at 960×540) this is ~5 minutes, not worth the pickling battle. Only use parallel when (a) all scene functions are pure and self-contained, (b) grids are instantiated inside `render_clip()`, and (c) you've tested with a single-worker pool first.

### Value Field Broadcasting — `g.cc` vs `g.rr`

`g.cc` is shape `(1, cols)`. `g.rr` is shape `(rows, 1)`. When you write:

```python
v = np.sin(g.cc * 0.03 + t) * 0.5      # shape (1, cols)
v += np.sin(g.rr * 0.04 - t) * 0.4     # BROADCASTING ERROR
```

The `+=` tries to broadcast `(rows, 1)` into `(1, cols)` — fails with `non-broadcastable output operand`. **Fix**: explicitly broadcast to full shape first:

```python
rows, cols = g.rows, g.cols
v = np.broadcast_to(np.sin(g.cc * 0.03 + t) * 0.5, (rows, cols)).copy()
v += np.broadcast_to(np.sin(g.rr * 0.04 - t) * 0.4, (rows, cols)).copy()
```

Values using `g.dist`, `g.angle`, `g.dist_n` are already full `(rows, cols)` — no issue.

### hsv2rgb_scalar Overflow

`(r + m) * 255` can produce values > 255 due to floating-point rounding. Assigning these to a uint8 array raises `OverflowError: Python integer 265 out of bounds for uint8`. **Fix**: always clamp:
...
### FeedbackBuffer Config Key Mismatch

When defining scene feedback config in a dict:

```python
"feedback": {"decay": 0.8, "blend": "screen", "opacity": 0.3, ...}
```

`FeedbackBuffer.apply()` expects `blend_mode`, not `blend`. The `render_clip()` loop must map keys:
...
### Setup in Constrained Containers

**ffmpeg not installed**: Download a static build instead of apt-get (which may need root):
```bash
curl -sL https://johnvansickle.com/ffmpeg/releases/ffmpeg-release-amd64-static.tar.xz -o ffmpeg.tar.xz
tar xf ffmpeg.tar.xz && cp ffmpeg-*-static/ffmpeg ffmpeg-*-static/ffprobe /workspace/
```

**pip install fails with permission/space errors**: When `/tmp` is mounted `noexec` or the default pip cache dir is read-only, install to the workspace:
```bash
TMPDIR=/workspace/.tmp pip3 install --no-cache-dir --target=/workspace/pylibs numpy Pillow scipy
